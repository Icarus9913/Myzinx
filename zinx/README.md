    ...
    --玩家的业务
        ...
        -玩家上线
            ...
            --实现Player玩家模块
                -属性：
                    1.玩家ID
                    2.连接信息(玩家用于和对应客户端通信的connection)
                    3.玩家当前坐在的坐标
                -方法:
                    1.创建一个玩家的方法
                        -给玩家生成一个玩家ID
                        -创建一个玩家对象
                    2.玩家可以和客户端通信的发送消息的方法
                        -将msg的proto格式进行序列化改成二进制
                        -通过zinx框架提供sendMsg将数据进行TLV格式的打包发包
            --实现上线业务功能
                -给server注册一个创建连接之后的Hook函数
                    1.给客户端发送MsgID：1的消息
                    2.给客户端发送MsgID：200的消息
                -给Player提供两个方法
                    1.将PlayerID同步给客户端 -->func (p *Player)SyncPid()
                    2.将Player的上线的初始位置同步给客户端 -->func (p *Player)BroadCastStartPosition()
            --测试上线功能
        -世界聊天
            --proto3联通协议的定义
            --聊天业务的实现
                ...
                2.将聊天的数据广播给全部在线玩家
                  创建一个世界管理模块
                    -作用：可以得到世界的全部玩家信息和全部的AOI地图信息
                    -属性：
                        1)AOIManager 当前世界地图AOI的管理模块
                        2)当前全部在线的Player集合
                        3)保护Players集合的锁
                    -方法：
                        1)初始化方法
